...
namespace SANDBOX_ClubMembershipApplication.FieldValidators
{
    public class UserRegistrationValidator:IFieldValidator
    {
        ...
        public void InitialiseValidatorDelegates()
        {
            ...
            _compareFieldsValidDel = CommonFieldValidatorFunctions.FieldCompareValidDel;
        }
        ...
        private bool ValidField(int fieldIndex, string fieldValue, string[] fieldArray, out string fieldInvalidMessage)
        {
            ...
            switch (userRegistrationField)
            {
                ...
                case FieldConstants.UserRegistrationField.PhoneNumber:
                    fieldInvalidMessage = (!_requiredValidDel(fieldValue)) ? $"You must enter a value for field:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : "";
                    fieldInvalidMessage = (fieldInvalidMessage == "" && !_patternMatchValidDel(fieldValue, CommonRegularExpressionValidationPatterns.PL_PhoneNumber_RegEx_Pattern)) ? $"You did not enter a valid PL phone number{Environment.NewLine}" : fieldInvalidMessage;
                    break;
                ...
                case FieldConstants.UserRegistrationField.PostCode:
                    fieldInvalidMessage = (!_requiredValidDel(fieldValue)) ? $"You must enter a value for field:{Enum.GetName(typeof(FieldConstants.UserRegistrationField), userRegistrationField)}{Environment.NewLine}" : "";
                    fieldInvalidMessage = (fieldInvalidMessage == "" && !_patternMatchValidDel(fieldValue, CommonRegularExpressionValidationPatterns.PL_Post_Code_RegEx_Pattern)) ? $"You did not enter a valid PL post code{Environment.NewLine}" : fieldInvalidMessage;
                    break;
                default:
